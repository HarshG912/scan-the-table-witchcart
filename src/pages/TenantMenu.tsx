...
export default function TenantMenu() {
  ...
  const [activeTab, setActiveTab] = useState<"menu" | "track">("menu");
  ...
  // Store cart in localStorage with tenant context
  useEffect(() => {
    if (tenantId && tableNumber) {
      localStorage.setItem(`cart_${tenantId}_${tableNumber}`, JSON.stringify(cart));
    }
  }, [cart, tenantId, tableNumber]);
    ...
    <div className="min-h-screen bg-background pb-24 animate-fade-in">
      <header className="sticky top-0 z-40 bg-primary text-primary-foreground shadow-lg">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold">{settings?.restaurant_name || "Restaurant"}</h1>
      ...
      <div className="max-w-7xl mx-auto">
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as "menu" | "track")} className="w-full">
          <TabsList className="sticky top-[72px] left-0 right-0 w-full grid grid-cols-2 h-14 rounded-none border-b shadow-md z-30 bg-background">
            <TabsTrigger value="menu" className="h-full text-base font-semibold">
              Menu
            </TabsTrigger>
            <TabsTrigger value="track" className="h-full text-base font-semibold">
              Track Order
            </TabsTrigger>
          </TabsList>
